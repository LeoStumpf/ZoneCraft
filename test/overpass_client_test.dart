...
void main() {
  ...
  group('retry policy', () {
    ...
    test('after its retries a rejecting instance is abandoned', () async {
      ...
      expect(
        tried.where((t) => t == overpassEndpoints.first),
        hasLength(2),
    ...
    test(
      'a timeout does NOT retry the same instance — it is slow, not busy',
      () async {
        final tried = <String>[];
        final out = await post(
          mock((req) async {
    ...
    test(
      'every instance rejecting reports busy, never blames the connection',
      () async {
        var calls = 0;
        final out = await post(
          mock((_) async {
  ...
  group('streaming', () {
    test(
      'an oversized body is abandoned mid-stream, not downloaded whole',
      () async {
        var delivered = 0;
    ...
    test('progress names which instance, and which try', () async {
      ...
      final contacts = seen
          .where((p) => p.stage == OverpassStage.contacting)
          .toList();
  ...
  group('cancellation', () {
    ...
    test('a cancelled request is not a failed one', () async {
      ...
      expect(
        calls,
        0,
        reason: 'cancelled before the wire, so nothing was sent',
      );
    });

    test('cancelling while an instance is silent gives up immediately', () async {
  ...
  group('describeImportTally', () {
    ...
    test('an all-duplicate import says so — the map does not change', () {
      expect(
        describeImportTally(
          const ImportTally(added: 0, skipped: 8),
          'stations',
        ),
