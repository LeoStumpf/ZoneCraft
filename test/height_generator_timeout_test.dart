...
void main() {
  ...
  http.Client stalled() => MockClient((_) => Completer<http.Response>().future);

  test(
    'queryElevation gives up on a stalled connection instead of hanging',
    () async {
      final result =
  ...
  test(
    'generateHeightRegion fails with a message, not a hang',
    () async {
      ...
      final region = (await repo.watchAllHeightRegions().first).firstWhere(
        (r) => r.id == regionId,
      );
