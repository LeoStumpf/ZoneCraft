...
// control). When that file is absent — local dev, CI without secrets — we fall
// back to the debug key so `flutter run --release` and scripts/build.sh still work.
...
android {
    ...
    buildTypes {
        release {
            ...
            // the debug key (so release builds without the keystore still run).
